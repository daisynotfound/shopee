...
<div><a href="{{ route('home') }}" style="text-decoration: none;">{{
__('Beranda') }}</a></div>
<div><a href="{{ route('transaksi.cart') }}" style="text-decoration:
none;">{{ __('Cart Tiket') }}</a></div>
</div>
<div class="card-body">
@if(session('success'))
...
<td style="color: {{ $transaksi->status == 'Selesai' ?
'green' : ($transaksi->status == 'Pending' ? 'red' : 'black') }};">
{{ $transaksi->status }}
</td>
